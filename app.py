...
def search():
    ...
    if request.method == "POST":
        ...
        if request.files["image"]:
            ...
            features1 =list(np.array(ht.extract_features(image_read)))
            ...
            # loaded_r = jsonify(json.loads(r))
             ...
             # perform the search
            searcher1 = Searcher(INDEX)
            results1 = searcher1.search(features1)
            # loop over the results, displaying the score and image name
            for (score, resultID) in results1:
                RESULTS_ARRAY1.append(
                {"image": str(resultID), "score": str(score)})     
            # return success
            results1 = RESULTS_ARRAY1[::]
            r1 = json.dumps(results1)
            # loaded_r = jsonify(json.loads(r))




            return render_template("index.html", jsonResult=json.loads(r), jsonResult1=json.loads(r1), image=image.filename)
            # return loaded_r
        else:
            return render_template('index.html')
